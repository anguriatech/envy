...
## Recent Changes
- 016-interactive-tui (banner policy, 2026-09-04): full gradient banner opens on terminals ≥32 rows; compact identity strip (` ENVY · v{version} · {workspace}`, bare wordmark <40 cols) otherwise; `B` toggles for the session (FR-062, `banner.rs`); `App::new` takes `expanded_banner` computed from terminal size in `run()`.
