...
- [X] T091 [US7] Banner default policy (FR-062): full gradient logo opens on terminals ≥32 rows, compact identity strip (` ENVY · v{version} · {workspace}`, wordmark-only under 40 cols) otherwise; `B` toggles for the session; strip builder unit-tested; verified at 40 and 24 rows via pyte-replayed pty.
