...
// ---------------------------------------------------------------------------
// cmd_key_export / cmd_key_import — master key recovery kit
// ---------------------------------------------------------------------------

/// Default recovery file name when `--output` is not given.
const DEFAULT_RECOVERY_FILE: &str = "envy-key-recovery.json";

/// JSON `kind` tag embedded in every recovery file, checked on import so a
/// stray unrelated JSON file produces a clear error instead of a confusing
/// deserialization failure.
const RECOVERY_FILE_KIND: &str = "envy-master-key-recovery";

/// Resolves the passphrase protecting a master-key recovery file.
///
/// Distinct from `ENVY_PASSPHRASE` (which protects `envy.enc`) — these guard
/// different secrets and mixing the two env vars up would be a genuinely
/// confusing failure mode, so a dedicated variable name is worth the small
/// duplication versus `resolve_passphrase`.
///
/// # Errors
/// - [`CliError::PassphraseInput`] if the passphrase is empty, the env var is
///   whitespace-only, or the terminal prompt fails (no TTY, Ctrl-C).
fn resolve_recovery_passphrase(
    prompt: &str,
    confirm: bool,
) -> Result<zeroize::Zeroizing<String>, CliError> {
    if let Ok(val) = std::env::var("ENVY_KEY_RECOVERY_PASSPHRASE") {
        if !val.trim().is_empty() {
            return Ok(zeroize::Zeroizing::new(val));
        }
        return Err(CliError::PassphraseInput(
            "ENVY_KEY_RECOVERY_PASSPHRASE is set but contains only whitespace".into(),
        ));
    }
...
/// Writes an encrypted backup of the local master key to `output`
/// (default: `./envy-key-recovery.json`).
///
/// Reuses the exact same Argon2id + AES-256-GCM envelope as `envy.enc`
/// (`crypto::artifact::seal_envelope`) — no new cryptographic primitive is
/// introduced for this feature.
///
/// # Errors
/// - [`CliError::KeyRecoveryFileExists`] if `output` already exists (never
///   silently overwritten).
/// - [`CliError::PassphraseInput`] on prompt failure or empty passphrase.
pub(super) fn cmd_key_export(output: Option<&Path>) -> Result<(), CliError> {
    let output_path = output
        .map(Path::to_path_buf)
        .unwrap_or_else(|| std::path::PathBuf::from(DEFAULT_RECOVERY_FILE));

    if output_path.exists() {
        return Err(CliError::KeyRecoveryFileExists(
    ...
    let master_key = crate::crypto::get_or_create_master_key()
        .map_err(|e| CliError::VaultOpen(e.to_string()))?;
    let hex = crate::crypto::encode_key_hex(&master_key);

    let passphrase = resolve_recovery_passphrase(
        "Recovery passphrase (protects this file \u{2014} store it separately from the file itself)",
        true,
    )?;

    let mut secrets = std::collections::BTreeMap::new();
    secrets.insert("master_key".to_string(), zeroize::Zeroizing::new(hex));
    let payload = crate::crypto::ArtifactPayload { secrets };
    let envelope = crate::crypto::seal_envelope(&passphrase, &payload)
        .map_err(|e| CliError::PassphraseInput(e.to_string()))?;
    ...
    let json_bytes =
        serde_json::to_vec_pretty(&doc).map_err(|e| CliError::Output(e.to_string()))?;

    // `create_new(true)` guards against a TOCTOU overwrite between the
    // existence check above and this write.
    {
...
/// Restores the master key from a recovery file into the OS Credential Manager.
///
/// If a local vault already exists, this can make it permanently unreadable
/// (unless the imported key happens to match the one it was created with) —
/// interactive mode asks for confirmation; `force` skips the prompt for
/// scripted use.
///
/// # Errors
/// - [`CliError::FileNotFound`] if `input` cannot be read.
/// - [`CliError::KeyRecoveryFileInvalid`] if the file isn't a recovery file
///   this build understands.
/// - [`CliError::PassphraseInput`] on wrong passphrase, prompt failure, or
///   empty passphrase.
/// - [`CliError::Aborted`] if the user declines the confirmation prompt.
pub(super) fn cmd_key_import(input: &Path, force: bool) -> Result<(), CliError> {
    let content = std::fs::read_to_string(input)
        .map_err(|e| CliError::FileNotFound(input.display().to_string(), e.to_string()))?;
    let doc: serde_json::Value = serde_json::from_str(&content)
        .map_err(|e| CliError::KeyRecoveryFileInvalid(e.to_string()))?;

    if doc.get("kind").and_then(|v| v.as_str()) != Some(RECOVERY_FILE_KIND) {
        return Err(CliError::KeyRecoveryFileInvalid(format!(
            "not an envy master-key recovery file (expected \"kind\": \"{RECOVERY_FILE_KIND}\")"
        )));
    }
    let envelope: crate::crypto::EncryptedEnvelope = doc
        .get("envelope")
        .cloned()
        .map(serde_json::from_value)
        .transpose()
        .map_err(|e: serde_json::Error| CliError::KeyRecoveryFileInvalid(e.to_string()))?
        .ok_or_else(|| CliError::KeyRecoveryFileInvalid("missing \"envelope\" field".into()))?;

    let passphrase = resolve_recovery_passphrase("Recovery passphrase", false)?;
    let payload = crate::crypto::unseal_envelope(&passphrase, "master-key-recovery", &envelope)
        .map_err(|_| {
            CliError::PassphraseInput(
                "wrong recovery passphrase, or the recovery file is corrupted".into(),
            )
        })?;
    let hex = payload
        .secrets
        .get("master_key")
        .ok_or_else(|| CliError::KeyRecoveryFileInvalid("missing master_key entry".into()))?;
    let key_bytes = crate::crypto::decode_key_hex(hex)
        .map_err(|e| CliError::KeyRecoveryFileInvalid(e.to_string()))?;
    ...
    if vault_path.exists() && !force {
        ...
        if !stdin_is_tty {
            return Err(CliError::Aborted(
                "a local vault already exists; re-run with --force to confirm you intend to \
                 replace its master key (non-interactive mode requires explicit confirmation)"
                    .into(),
            ));
        }
        let theme = dialoguer::theme::ColorfulTheme::default();
        let confirmed = dialoguer::Confirm::with_theme(&theme)
            .with_prompt(
                "A local vault already exists. Importing a different master key will make it \
                 permanently unreadable unless this is the SAME key it was created with. Continue?",
            )
            .default(false)
            .interact()
            .unwrap_or(false);
        if !confirmed {
            return Err(CliError::Aborted("key import cancelled by user".into()));
        }
    }
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // cmd_key_export / cmd_key_import
    //
    // These tests deliberately never reach `crate::crypto::get_or_create_master_key`
    // or `crate::crypto::set_master_key` — both touch the REAL OS keyring, which
    // would mutate the developer machine's actual envy master key. Every test
    // here exercises a code path that returns before that call (file-exists
    // guard, malformed-file validation, or a wrong passphrase failing to unseal).
    // The keyring integration itself is covered by the #[ignore]'d tests in
    // `crypto::keyring`, run manually with a live keyring daemon.
    // -----------------------------------------------------------------------

    #[test]
    fn cmd_key_export_refuses_to_overwrite_existing_file() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let output = tmp.path().join("recovery.json");
        std::fs::write(&output, "not touched").expect("write pre-existing file");

        let result = super::cmd_key_export(Some(&output));
        assert!(
            matches!(result, Err(CliError::KeyRecoveryFileExists(_))),
            "must refuse to overwrite an existing file, got: {:?}",
            result
        );
        assert_eq!(
    ...
    #[test]
    fn cmd_key_import_missing_file_returns_file_not_found() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let missing = tmp.path().join("does-not-exist.json");

        let result = super::cmd_key_import(&missing, true);
        assert!(
            matches!(result, Err(CliError::FileNotFound(_, _))),
            "missing recovery file must return FileNotFound, got: {:?}",
            result
        );
    }
    ...
    fn cmd_key_import_rejects_wrong_kind() {
        ...
        assert!(
            matches!(result, Err(CliError::KeyRecoveryFileInvalid(_))),
            "wrong 'kind' must be rejected before any decryption is attempted, got: {:?}",
            result
        );
    }

    #[test]
    fn cmd_key_import_rejects_malformed_json() {
        let tmp = tempfile::tempdir().expect("tempdir");
        ...
        assert!(
            matches!(result, Err(CliError::KeyRecoveryFileInvalid(_))),
            "malformed JSON must return KeyRecoveryFileInvalid, got: {:?}",
            result
        );
    }

    #[test]
    fn cmd_key_import_wrong_passphrase_never_touches_keyring() {
        let _guard = ENV_LOCK.lock().unwrap();

        // Build a recovery file directly via the crypto layer -- this test
        // must NOT call cmd_key_export, since that touches the real keyring.
        let mut secrets = std::collections::BTreeMap::new();
        secrets.insert(
        ...
        std::fs::write(&path, serde_json::to_vec(&doc).expect("serialize")).expect("write file");

        // SAFETY: single-threaded access serialised by ENV_LOCK above.
        unsafe { std::env::set_var("ENVY_KEY_RECOVERY_PASSPHRASE", "wrong-passphrase") };
        let result = super::cmd_key_import(&path, true);
        unsafe { std::env::remove_var("ENVY_KEY_RECOVERY_PASSPHRASE") };

        assert!(
            matches!(result, Err(CliError::PassphraseInput(_))),
            "wrong passphrase must fail at unseal, before set_master_key is ever called, got: {:?}",
            result
        );
    }
