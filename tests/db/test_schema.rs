...
/// (b) PRAGMA user_version equals 3 after the first open.
///
/// Verifies that `run_migrations` ran the V1, V2, and V3 migrations and set
/// the schema version to 3.
#[test]
fn test_schema_version_is_one_after_open() {
    ...
    assert_eq!(version, 3, "user_version must be 3 after initial migration");
}
...
fn test_reopen_is_idempotent() {
    ...
    // First open — runs migrations, sets user_version = 3.
    {
        let vault = Vault::open(tmp.path(), &DUMMY_KEY).expect("first open must succeed");
        vault.close().expect("close must succeed");
    }

    // Second open — should succeed without error and leave user_version = 3.
    let vault =
    ...
    assert_eq!(version, 3, "user_version must still be 3 on re-open");
}
